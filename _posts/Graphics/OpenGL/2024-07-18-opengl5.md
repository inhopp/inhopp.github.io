...
Vertex의 데이터를 Texture 형태로 저장하는 방식의 장점이 무엇일까 생각해 보았다. 내가 생각한 내용들은 interpolation이 간단하고, 모델을 생성할 때 아트분들 생산성에 관련이 있나? 정도였다. 혹시 더 있을까 GPT에게 물어봤는데 상당히 많은 내용이 있었다.
