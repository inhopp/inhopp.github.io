...
Hybrid Simulation의 동작을 간략하게 이야기하면 위 그림과 같다. `SortElemnts`라는 구조체 배열에는 그리드 셀의 인덱스를 의미하는 key, 파티클 배열의 인덱스를 의미하는 value 값이 들어있다. 매 프레임 Bitonic sort를 이용해 SortElements 배열을 정렬하면 특정 그리드 셀에 포함되는 파티클들을 빠르게 구할 수 있다. 그리드 기반으로 계산한 물리량을 해당 그리드에 포함된 파티클에 적용시키고, 주변 파티클들과 입자 기반 연산을 수행한다. 그 후 움직인 파티클들을 기준으로 다시 그리드 셀의 정보를 수정해주어야 한다. 
