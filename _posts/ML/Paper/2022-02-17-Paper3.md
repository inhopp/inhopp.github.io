...
이때 그림의 아래 row는 테두리에 16 pixel씩 padding한 것이다. 패딩한 영역의 픽셀 값은 주변 값들의 평균값이다. Padding 추가시 mAP가 3-5% 증가했다.
...
2. Annotated detection data로 **fine-tuning**
   ...
   - +1 : backgorund
   - 즉, 이상한 영역이 들어오면 '이건 배경이야' 라고 training
...
기존의 전략은 IoU 0.5값을 기준으로 positive, negative를 구분하는 것이다. 그 후 positivie example은 해당 class로, negative example은 background class로 training 한다. 하지만 이렇게 하려니 문제가 발생했다.
...
따라서 저자들은 IoU 0.5값을 기준으로 postive/negative를 나누지 않았다. 중간에 회색지대 (gray zone)를 만들고 이들을 훈련데이터에서 제외시켰다. softmax는 positive가 아니면 무조건 negative로 가기 때문에 이러한 작업이 불가능하다.
...
![17](https://user-images.githubusercontent.com/96368476/154723339-1a30c3c2-09d9-45bc-9d16-5141df8aa7b4.png){: width="70%" height="80%" .align-center}

- 1. training set 개수를 5000개 늘려주니 성능 향상
- 2. training set 5000개 더 추가했더니 영향 없없음
...
우리는 Computer Vision 분야에 Deep Learning 기술을 성공적으로 결합시켰다. 아주 자랑스럽다. 
