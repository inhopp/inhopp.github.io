...
**[Image Style Transfer Using Convolutional Neural Networks - paper](https://github.com/inhopp/inhopp/files/8025046/Image.Style.Transfer.paper.pdf)**
{: .notice--primary}
...
위 그림은 Content Image를 각각 Conv2_2 필터, Conv4_2 필터를 이용해 reconstruction한 것이다. Conv2_2 필터를 이용한 경우, 확대한 사진을 보면 content image의 저차원 특징이 너무 선명해서 단순히 색깔만 입혀놓은 듯 하다. 반면 Conv4_2 필터를 이용하니 해상도가 떨어지지만, 오히려 더 자연스럽게 스타일이 표현된 듯하다. 이러한 이유로 conv4_1 필터를 이용하는 듯하다.
...
![17](https://user-images.githubusercontent.com/96368476/153115154-f2fad544-7f4b-4561-a552-bf5aaeeb9595.jpg){: width="70%" height="80%" .align-center}
![15](https://user-images.githubusercontent.com/96368476/153115141-c0d3d04b-d9b9-48ae-aae1-1461080df2d8.jpg){: width="70%" height="80%" .align-center}
![18](https://user-images.githubusercontent.com/96368476/153115582-57b8f028-3944-413a-b793-fd267e76bcfa.jpg){: width="70%" height="80%" .align-center}
...
저자들이 제시한 스타일(feature map들의 상관관계)의 정의는 완벽하지 않다. 고흐의 '별이 빛나는 밤' style을 예로 들어보자. 사람이 보기에 그림의 '별'은 스타일이 아닌 object이다. 하지만 본 논문의 style transfer에서는 별을 스타일로 인식한다. 아직 개선해야할 일이 남아있다. 인간이 짱이다.
...
일본 여행에 좋은 기억이 많아서 일본 풍경으로 tutoral 코드를 돌려봤다(사람 얼굴로 돌려보니 결과가 조금 흉측하다). tensorflow, pytorch 두 버전 모두 해봤는데 개인적으로 pytorch가 더 보기 편했다. 그리고 gpu 많이 쓰니까 코랩에서 못쓰게 막더라.
