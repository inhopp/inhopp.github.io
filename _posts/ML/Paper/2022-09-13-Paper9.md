...
Fast R-CNN은 mini-batch sampling 기법도 크게 개선했다. 기존의 방식은 서로 다른 128장의 이미지에서 RoI를 샘플링하는 방법이었지만, 개선된 방식은 2장의 이미지에서 각 64개씩 RoI들을 샘플링했다. 이 비율은 저자들이 실험적으로 정한 비율인데, 이런 방식으로 진행하면 메모리를 훨씬 효율적으로 사용할 수 있다(feature map을 공유하므로). 너무 당연한 이야기라 드는 의문점은 왜 진작 하지 않았을까.. 
