...
앞서 봤듯이 CNN은 크게 Convolution Layer, Fully Connected Layer 두 파트로 나눌 수 있다. 이때 convolution의 과정을 생각해보면 fixed input size를 요구하지 않는다는 것을 알 수 있다. 아무 사이즈가 들어와도 convolution은 가능하다. 즉, fixed input size를 요구하는 범인은 CNN 저 끝에 있는 FC layer란 소리다. 
...
- Region proposal 이후 CNN은 FC layer 같은 아주 단순한 구조 (이미 특징을 뽑아놓았기 때문에)
- 최대 102배까지 속도가 빨라지더라!
